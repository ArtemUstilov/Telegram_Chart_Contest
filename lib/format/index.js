...
export const formatValue = (value)=>{
  ...
  let suffix = SUFFIXES.find(x => log > x.exp);
  const baseUnit = Math.pow(10, suffix.exp);
  return ({ value, label: `${Math.round(value / baseUnit)}${suffix.label}` });
};
...
export const dateFormatter = (maxLabels) => {
    ...
    const format = (min, max) => {
        ...
        // let newLabels = [];
        // const diff = (max - min)/7;
        // for(let i = 0; i < 8; i++){
        //     newLabels.push({value: min + diff*i, label: formatDateMonthDay(min + diff*i)})
        // }
