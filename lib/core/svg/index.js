import { createSVGAxis } from '../../axis';
...
export const createSVGGraph = function (element, xAxisFormatter, yAxisFormatter) {
  let axis;
  if (xAxisFormatter && yAxisFormatter) {
    axis = createSVGAxis(element, xAxisFormatter, yAxisFormatter);
  }
  const svgElement = createSVGElement('svg');
  
  svgElement.setAttributeNS(null, 'height', element.clientHeight);
  svgElement.setAttributeNS(null, 'width', element.clientWidth);
  svgElement.setAttributeNS(null, 'viewBox', '0 0 1 1');
  svgElement.setAttributeNS(null, 'preserveAspectRatio', 'none');
  svgElement.setAttributeNS(null, 'style', 'position: absolute');
  ...
  const refresh = function refresh() {
    ...
    polylines = lines.map((item) => {
      ...
      polyline.setAttributeNS(null, 'stroke-width', '2.5');
