...
import { reduceGraph, findClosestValueIndex } from "../../index";
...
export const createPercentagedGraph = function (parent, customOptions) {
  ...
  if (options.createNameplates) {
    table = createSVGTable({...options, type: 'percentaged'}, data);
  ...
  const setData = function setData(newData) {
    ...
    let percentagedData = new Array(data.lines.length).fill(0).map(x => ({color: '', name: '', values: []})
    )
