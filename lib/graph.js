...
const createGraphUpdater = (myGraph, data, totalMinX, totalMaxX) => {
  ...
  const reduceData = (graph, data, minX, maxX) => {
    graph.setData(reduceGraph(data, 1000, minX, maxX, 0.5));
  };
  const reduceDataDebounced = debounce(reduceData, 100);
  ...
  const draw = () => {
    ...
    const data = myGraph.getData();
    const now = performance.now();
    if (animation || prevScale !== scale || data !== prevData) {
      if (animation && scale) {
