...
const createGraphUpdater = (myGraph, data, totalMinX, totalMaxX) => {
  ...
  const updater = (min, max) => {
    ...
    const maxY = Math.max(...data.lines.map(line => findMaximum(line.values, minXIndex, maxXIndex))) / MAX_BOUND;
    ...
    if (animation || prevScale !== scale || data !== prevData) {
      ...
      if (animation && scale) {
        ...
        if (time < 1) {
          ...
          const maxY = scale.maxY + (step / left) * diff * diff * Math.sign(diff) * ANIMATION_SPEED;
