export const dragMaster = (function () {
  ...
  function stretchLeft(e) {
    ...
    let width =  dragObject.getBoundingClientRect().right - mWidth*2 - left - leftEdge;
    console.log(left + width + leftEdge, dragObject.getBoundingClientRect().right);
    if (width <= minWidth) {
      width = minWidth;
  ...
  return {
    makeDraggable: function (element, left, width, minW, markersWidth, size, colorsState, onDrag) {
      ...
      bgPreview.setAttribute('id', 'bg-preview');
      bgElem = document.createElement('div');
      bgElem.setAttribute('id', 'bg-preview-active');
